...
public class DynamicTags {
    ...
    public static final TagKey<Item> BLACKLISTED = TagKey.of(Registry.ITEM.getKey(), Identifier.tryParse(Main.MOD_ID + ":blacklisted"));
    static {
        DYNAMIC_TAGS.put(BLACKLISTED, () -> SimpleConfig.BLACKLISTED_PICKAXES.getValue().stream()
                .map(p -> Registry.ITEM.get(Identifier.tryParse(p))).collect(Collectors.toSet()));
    }
